...
class SeatPairController extends ApiController
{
    ...
    public function update(Request $request, SeatPair $seatPair): JsonResponse
    {
        ...
        $update = [];
        if (array_key_exists('pairSequence', $validated)) {
            $update['pair_sequence'] = $validated['pairSequence'];
        }
        if (array_key_exists('seatAId', $validated)) {
            $update['seat_a_id'] = $validated['seatAId'];
        }
        if (array_key_exists('seatBId', $validated)) {
            $update['seat_b_id'] = $validated['seatBId'];
        }
