@if($inline)
    ...
    <div class="flex items-center gap-2">
        <button
            ...
            class="rounded px-2 py-1 text-sm font-semibold transition {{ $locale === 'pt-PT' ? 'bg-primary-100 text-primary-700 dark:bg-primary-900 dark:text-primary-300' : 'text-gray-500 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800' }}"
        >
            PT
        </button>
        <button
            ...
            class="rounded px-2 py-1 text-sm font-semibold transition {{ $locale === 'en-US' ? 'bg-primary-100 text-primary-700 dark:bg-primary-900 dark:text-primary-300' : 'text-gray-500 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800' }}"
        >
            EN
        </button>
    </div>
@else
    ...
    <div class="relative inline-block" x-data="{ open: false }" @click.away="open = false">
        <button
            type="button"
            @click="open = !open"
            class="flex items-center justify-center gap-1.5 h-9 rounded-md text-sm font-medium transition-colors
        ...
        >
            ...
            <span class="hidden sm:inline text-xs font-semibold uppercase tracking-wide">{{ $locale === 'pt-PT' ? 'PT' : 'EN' }}</span>
