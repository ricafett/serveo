<div class="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 py-6">
    ...
    <div class="mb-6">
        @if ($activeSession)
            <div class="flex items-center gap-2 rounded-lg bg-primary-50 dark:bg-primary-900/20 px-4 py-3 text-sm font-medium text-primary-700 dark:text-primary-300">
                <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <span>{{ __('dashboard.active_session') }}: {{ $activeSession->name }}</span>
            </div>
        @else
            <div class="flex items-center gap-2 rounded-lg bg-amber-50 dark:bg-amber-900/20 px-4 py-3 text-sm font-medium text-amber-700 dark:text-amber-300">
                <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
                </svg>
                <span>{{ __('dashboard.no_session') }}</span>
            </div>
        @endif
    </div>
            ...
            >
                <div class="flex items-center gap-3 w-full">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <p class="mt-0.5 text-sm text-gray-500 dark:text-gray-400">{{ $tile['description'] }}</p>
                    </div>
