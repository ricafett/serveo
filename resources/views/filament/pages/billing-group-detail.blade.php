<x-filament-panels::page>
    <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
        <div class="rounded-lg border bg-white p-4 dark:bg-gray-900">
            <div class="text-xs text-gray-500 dark:text-gray-400">{{ __('billing.status') }}</div>
            <div class="text-lg font-semibold">{{ $group->status?->display_name ?? $group->status?->code }}</div>
        </div>
        <div class="rounded-lg border bg-white p-4 dark:bg-gray-900">
            <div class="text-xs text-gray-500 dark:text-gray-400">{{ __('billing.total_to_pay') }}</div>
            <div class="text-lg font-semibold">{{ number_format($charges, 2, ',', ' ') }} EUR</div>
            <div class="text-xs text-gray-500 dark:text-gray-400">{{ __('billing.paid') }}: {{ number_format($paid, 2, ',', ' ') }} EUR</div>
        </div>
        <div class="rounded-lg border bg-white p-4 dark:bg-gray-900">
            <div class="text-xs text-gray-500 dark:text-gray-400">{{ __('billing.open_balance') }}</div>
    ...
    <div class="mt-6 rounded-lg border bg-white p-4 dark:bg-gray-900">
        <h3 class="mb-3 text-base font-semibold">{{ __('billing.occupied_zones') }}</h3>
        @if ($group->occupiedZones->isEmpty())
            <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('billing.no_zones') }}</p>
        @else
    ...
    <div class="mt-6 rounded-lg border bg-white p-4 dark:bg-gray-900">
        <h3 class="mb-3 text-base font-semibold">{{ __('billing.orders') }}</h3>
        @if ($group->orderHeaders->isEmpty())
            <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('billing.no_orders') }}</p>
        @else
    ...
    <div class="mt-6 rounded-lg border bg-white p-4 dark:bg-gray-900">
        <h3 class="mb-3 text-base font-semibold">{{ __('billing.documents_payments') }}</h3>
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div>
                <div class="mb-1 text-xs uppercase text-gray-500 dark:text-gray-400">{{ __('billing.printed_bills') }}</div>
                ...
                @empty
                    <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('billing.no_bills') }}</p>
                @endforelse
            </div>
            <div>
                <div class="mb-1 text-xs uppercase text-gray-500 dark:text-gray-400">{{ __('billing.payments') }}</div>
